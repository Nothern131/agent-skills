...
```
用户: 做一个文件上传功能
Agent: [直接实现文件上传API，代码通过所有测试]
Agent: 文件上传功能已完成！
用户: 嗯...我想要的是拖拽上传，不是选择文件
Agent: [重新实现拖拽上传]
用户: 还有，上传后要显示缩略图
Agent: [再加缩略图]
用户: 哦对，还要支持批量上传
Agent: [继续加批量]
→ 反复修改4-5次，用户疲惫，Agent也疲惫
```

**问题：代码是对的，但不是用户要的。隐性需求在过程中才暴露。**

#### 有第五层防御（预期对齐 + 意图验证 + 渐进交付）

```
用户: 做一个文件上传功能
Agent: [预期对齐] 我理解您需要：
  - 目标：文件上传组件
  - 上传方式：拖拽还是选择文件？还是两者都要？
  - 文件类型：图片？文档？任意？
  - 上传后：需要预览/缩略图吗？
  - 批量：支持多文件吗？
用户: 拖拽+选择都要，主要是图片，要缩略图，支持批量
Agent: [MVP先行] 先实现核心：拖拽+选择上传单张图片+缩略图预览
→ 完成第一版
Agent: [意图验证] 第一版完成：可以拖拽或选择上传图片，上传后显示缩略图。
  这符合您的预期吗？
用户: 对，继续加批量
Agent: [第二版] 加入批量上传+进度条
→ 完成
用户: 完美
```

**结果：2次确认就完成，而不是4-5次反复修改。**

### 最后一公里防御的5条规则

| 规则 | 解决什么 | 核心动作 |
|------|----------|----------|
| 隐性需求挖掘 | 用户没说的需求 | 每步完成后自问5个检查项 |
| 用户意图验证 | 代码对了但不是要的 | 双环验证：内环代码正确性，外环意图匹配度 |
| 错误级联预防 | 修A坏B修B坏C | 修复复杂度预算2次上限，级联信号立即停止 |
| 交付节奏控制 | 一口气做完方向偏了 | MVP先行，强制用户确认检查点 |
| 预期对齐 | 做完了发现不是要的 | 开始前复述需求+确认边界+确认完成标准 |

**核心洞察：前四层解决"代码正确性"，第五层解决"做出来的东西是用户要的"。**
